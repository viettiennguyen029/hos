...
5. **Avalanche's public C-Chain RPC nodes (`api.avax-test.network`, `api.avax.network` — the defaults baked into `hardhat.config.ts`) reject `eth_estimateGas` queries against the `"pending"` block tag** with `ProviderError: state not available for pending block`. `@nomicfoundation/hardhat-ethers`'s `HardhatEthersProvider.estimateGas` defaults to `blockTag: "pending"` whenever a transaction's `gasLimit` isn't already set, and `HardhatEthersSigner._sendUncheckedTransaction` calls it on every deploy/write unless `gasLimit` is pre-populated. A network-level `gas: <number>` in `hardhat.config.ts` does **not** fix this — this version of hardhat-ethers never wires that config value into the signer's `_gasLimit`, so it's silently ignored. The actual fix is passing an explicit `gasLimit` (or, for `deployProxy`, `txOverrides: { gasLimit }`) at every deploy call site — see `contracts/scripts/deploy.ts`'s `DEPLOY_GAS_LIMIT` constant for the pattern. A fixed `gasPrice` in the network config is still worth setting (avoids a separate EIP-1559 fee-estimation quirk), but doesn't address this specific error on its own.
